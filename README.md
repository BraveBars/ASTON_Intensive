...
Set не допускает дубликатов. Основные реализации: HashSet (быстрая): O(1) для операций добавления/удаления. TreeSet (отсортированная): O(log n).
### 52.	Что знаете о коллекциях типа Queue?
Queue поддерживает последовательность элементов (FIFO). Основные реализации: LinkedList (FIFO). PriorityQueue (в порядке приоритета).
### 53.	Что знаете о коллекциях типа Map и их принципиальное отличие?
Map — структура для хранения пар "ключ-значение". Принципиальное отличие: Map не является наследником Collection.
